...
class agent:
    ...
    def update_q_table(self, state_list_vel, state_list_dis, act_list, reward_list):
        ...
        self.update_q_table(act_list[-1], state_list_vel[-1], state_list_dis[-1], q_list[-1])
        for step in range(2,len_st+1):
            ...
            self.update_q_table(act_list[current_step], state_list_vel[current_step], state_list_dis[current_step], q_list[current_step])
            
    def update_q_value(self, state_act, state_vel, state_dis, q_val):
        old_q = self.q_table[state_vel,state_dis,state_act]
        self.q_table = old_q + self.conf_learnrate * (q_val - old_q)
        return self.q_table[state_vel,state_dis,state_act]
...
#%% Environment class       
class env:
    def __init__(self, soc_init, st_vel_set, st_dis_set):
        self.r_reg = 0
        self.r_drv = 0
        self.r_saf = 0
        self.soc_old = soc_init
        self.set_r_coef()
    ...
    def get_reward_reg(self, data_soc):
        self.r_reg = self.rc_reg*(data_soc - self.soc_old)
        return self.r_reg
    ...
    def get_reward(self,data_vel,data_dis,data_soc,data_brk):
        r_reg = self.get_reward_reg(data_soc)
...
beh_driving.conf_forecast_dis = 150
...
env_brake = env(soc, vel_index, dis_index)
agent_mc = agent(1/len(act_index), act_index, vel_index, dis_index)
#%% 2. Simulation config
Ts = 0.01
sim_time = 200
...
for sim_step in range(len(sim_time_range)):
    ...
    # Driver assistant inputs - from reinforcement learning
...
#    t_load = kona_power.ModMotor.t_load
    ...
    if stStatic.state == 'Tl_stop':
        # 4. Reward define
        reward = env_brake.get_reward(veh_vel, rel_dis, soc, u_brk_filt)
    ...
    * Algorithm
    ...
    data_set = [veh_vel, veh_vel_set, u_acc_filt, u_brk_filt, pos_x, pos_y, pos_s,
                ...
                t_mot_reg, kona_body.t_brake, stStatic.state, 
